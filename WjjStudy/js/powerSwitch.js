...
;(function($, window, undefined) {
    ...
    $.powerSwitch = function(elements, options) {
     return    $(elements).powerSwitch(options);//这里可以不写，因为没有任何的实例化 new $.powerSwitch(this,opt),则该方法不会执行。但是这里没有，不代表客户端没有，调用者可以使用$('#element').powerSwitch({options})或者 new $.powerSwitch($(''),{options})
    };
    //方法定义
    $.extend($.powerSwitch, {
    ...
    $.fn.powerSwitch = function (options) {
        //一般会通过实例化 new $.powerSwitch(this,opt)来执行构造函数以初始化插件。这里没有写在构造函数中，而是单独来写，所以构造函数可以留空。
        // 默认参数
        var defaults = {
                    ...
                    if (params.classDisabled) {
                        ...
                        if (index == 0) {
                            ...
                            if (indexWill < 0) {
                                ...
                                $.powerSwitch.animation(null, null, $.extend({}, params, {
                                    animation: "none"
                                }));
